...
PageObject = Class.extend({
  ...
  /**
   * Performs a mousover event on the provided element.
  ...
  /**
   * Performs a mouseout event on the provided element.
   * Currently only works with css selectors
   *
   * @param selector {string} - css selector for the element to mouseover
   */
  unhover: function(selector) {
    if (typeof selector !== 'string') {
      throw new TypeError('Expected selector to be a string');
    }
  ...
  uploadFile: function(contents) {
    ...
    // Zip the file
    zip.file('temp', contents.toString('base64'), {base64:true});
    data = zip.generate({type:'base64'});
    fs.writeFileSync('/var/tmp/test2.zip', data, 'base64');
