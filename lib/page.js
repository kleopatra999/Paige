...
PageObject = Class.extend({
  ...
  /**
   * Open a url in a webdriver session
   *
   * @param url (optional) - the url to open. If no url is provided, the
   *                         default url for the current Page will be opened
   */
  open: function(url) {
  ...
  // switch context to a new Page
  switchTo: function(Page) {
    return new Page(this._config, this._session);
  },

  // switch context to a new page, and open page in the session
  redirectTo: function(Page) {
    return this.switchTo(Page).open();
  },

  /**
   * Execute arbitrary javascript in the current session
   *
  ...
  // Switch to a frame/iFrame context
  switchToFrame: function( frameId ) {
    this._session.switchTo().frame( frameId );
    return this;
  },

  // return the main context after switchToFrame()
  ...
  // Check whether an element exists in the current session
  ...
  // Find all elements described by the locator
  // @returns [WebElements]
  ...
  // Finds an element in a list based on a matcher
  ...
  /**
   * Wait for an element to be displayed in the current session
   *
   * @param element - the element to wait for
   * @param timeout (optional) - time in milliseconds to wait before erroring.
   */
  ...
  /**
   * Wait for an element to not be displayed in the current session
   *
   * @param element - the element to wait for
   * @param timeout (optional) - time in milliseconds to wait before erroring.
   */
  ...
  // Force element to be scrolled into view
  ...
  *
  * @param selector {String} - Paige selector of element
  * @param location {(String|{x: number, y: number})} - location to drag to,
  *   as either a Paige selector of another element or as an offset in pixels
  *
  */
  dragAndDrop: function(selector, location) {
    var element = this.find(selector),
        moveTo = location;

    if (typeof location === 'string') {
       moveTo = this.find(location);
    }

    new webdriver.ActionSequence(this._session)
    .dragAndDrop(element, moveTo)
    .perform();
