...
PageObject = Class.extend({
   ...
   * @param strict {Boolean} - if true, will verify case matches as well. By default, this is false
   */
  verifyContent: function(element, content, strict) {
    strict = strict || false;
    content = strict ? content : content.toLowerCase();
    ...
    if (typeof element === 'string' || typeof element[0] === 'string') {
      this.whenDisplayed(element).then(function() {
        this.find(element).getText().then(function(innerText) {
          innerText = strict ? innerText : innerText.toLowerCase();
