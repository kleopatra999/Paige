...
var Form = {
  ...
  enterInformation: function(formKey, data) {
    ...
    Object.keys(form.inputs).forEach(function(inputKey) {
      ...
      lookupFillIn(this, input.type).call(this, $form, input.selector, data[inputKey]);
    }.bind(this));
  ...
  fillInSelect: function($form, selector, data) {
    this.whenDisplayed(selector).then(function() {
      ...
      $form.find(selector).find('[value="' + data + '"]').click();
    });
  },

  // <input type="checkbox"></input>
  fillInCheckbox: function($form, selector) {
    this.whenDisplayed(selector).then(function() {
      $form.find(selector).click();
    });
  },
  ...
  fillInRadio: function($form, selector) {
    this.whenDisplayed(selector).then(function() {
      $form.find(selector).click();
    });
  }
